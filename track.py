...
def blog_index():
    ...
    return bottle.template('track_template', dict(username=username))
...
def present_signup():
    return bottle.template("signup",
        {'username':'', 'password' : "", "password_error":"", "name_error":"" , 'name':"",
         "phone":"", "phone_error":"", "email":"", "username_error":"", "email_error":"","verify_error" :""})

#                           dict(username="", password="",
#                                password_error="", name="", name_error="",
...
# set these up in case we have an error case
    ...
    if validate_signup(username, name, password, verify, email, phone, errors):

#    if validate_signup(username, name, password, verify, email, errors):

#        if not users.add_user(username, name, password, email, phone):
        if not users.add_user(username, name, password, email):
...
def validate_signup(username, name, password, verify, email, phone, errors):
#def validate_signup(username, name, password, verify, email, errors):
    USER_RE = re.compile(r"^[a-zA-Z0-9_-]{3,20}$")
    NAME_RE = re.compile(r"^[a-zA-Z]{3,20}$")
    PASS_RE = re.compile(r"^.{3,20}$")
    EMAIL_RE = re.compile(r"^[\S]+@[\S]+\.[\S]+$")
    PHONE_RE = re.compile(r"^[0-9]{9,10}$")
    ...
    if not NAME_RE.match(name):
        errors['name_error'] = "invalid username,try just letters and numbers"
        print "name error"
        return False
...
#    if not PHONE_RE.match(phone):
 #       errors['phone_error'] = "invalid Phone number. try 10 digit number "
 #       return False
    return True
